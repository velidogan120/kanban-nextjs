...
const Comment = ({column,comment,id}:{column:IColumn,comment:IComment,id:string}) => {
  ...
  const { attributes, listeners, setNodeRef, transform , isDragging } = useDraggable({
    id: id, 
    data: { id: id, column: comment.column }
  });
        ...
        <Card.Grid className='card' key={id} >
          <div className='comment'>
            <div className='comment_global_bg'>
              ...
              {step == 2 ? <button role='button' className='comment-item_btn_box' onClick={() => vote(id)} disabled={btnDisabled}>{comment.vote}<CheckCircleFilled className='comment-item_btn' /></button>:""}
              {column.label == "actions" || step == 1 ? <div className='comment-item_btn_box deleteBtn' onClick={() => deleteCard(id)}><CloseCircleFilled className='comment-item_btn' /></div> :""}     
            </div>
          </div>
        </Card.Grid>
      </div>
